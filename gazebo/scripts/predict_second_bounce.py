...
"M2_clean" runs the same pipeline on NOISE-FREE observations — its error
isolates the bounce-model mismatch vs DART's contact solver (estimation error
is ~0 there), i.e. the floor any M2 estimator inherits from the bounce model.
...
def main():
    ...
    lines = ["# M2: second-touchdown prediction vs Gazebo truth\n",
             ...
             "M2_clean = noise-free fit -> pure bounce-model mismatch vs DART.\n",
             "| condition | truth2 x,y (m) | M2_M1 | M2_conf | M2_clean |",
             "|---|---|---|---|---|"]
    for rundir in a.rundirs:
        rundir = rundir.rstrip("/")
        name = os.path.basename(rundir)
        if not os.path.exists(os.path.join(rundir, "bounces.csv")):
            continue
        res = eval_dir(rundir, a.seeds, a.fps, a.sigma0, a.alpha, a.bad_frac)
        if res is None:
            lines.append(f"| {name} | (1st touchdown off table) | — | — | — |")
            continue
        truth2, errs, clean = res
        cells = [f"{np.nanmean(np.array(errs[m])):.2f} ± "
                 f"{np.nanstd(np.array(errs[m])):.2f}"
                 for m in ("M2_M1", "M2_conf")]
        lines.append(f"| {name} | {truth2[0]:.4f}, {truth2[1]:.4f} | "
                     f"{cells[0]} | {cells[1]} | {clean:.2f} |")
