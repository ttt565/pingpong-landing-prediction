...
def main():
    ...
    # ---------------------------------------------------------------
    methods_full = ["M0", "M1", "M3_conf", "M3_oracle", "M4"]
    ...
    Hs, e_m1, e_or, e_cf, gor, gcf = [], [], [], [], [], []
    log(f"    {'bad_frac':>8s} {'H':>5s} {'M1':>6s} {'M3orac':>7s} {'M3conf':>7s} "
        f"{'gapOr':>7s} {'gapCf':>7s}  (cm)")
    for bf, (tag, _) in zip(bad_fracs, conds):
        r = R[tag]
        H = np.mean([x["_H"] for x in r])
        m1, orc, cf = col(r, "M1"), col(r, "M3_oracle"), col(r, "M3_conf")
        Hs.append(H)
        e_m1.append(np.nanmean(m1)); e_or.append(np.nanmean(orc)); e_cf.append(np.nanmean(cf))
        d_or = boot_gap_ci(m1, orc); d_cf = boot_gap_ci(m1, cf)
        gor.append(d_or); gcf.append(d_cf)
        log(f"    {bf:8.2f} {H:5.2f} {np.nanmean(m1):6.2f} {np.nanmean(orc):7.2f} "
            f"{np.nanmean(cf):7.2f} {d_or[0]:+7.2f} {d_cf[0]:+7.2f}")
    _hetero_fig(bad_fracs, Hs, e_m1, e_or, e_cf, gor, gcf)
    ...
    log(f"  - deployable M3_conf recovers a fraction of the oracle gap.")
    log(f"  => GO only if the real perception pipeline shows high within-arc H")
    log(f"     (measure it from TrackNet). Otherwise M3's benefit is below the floor.")
...
def _bar_fig(res, methods, Hbar):
    ...
    colors = ["#888", "#1f77b4", "#2ca02c", "#9467bd", "#aaa"]
...
def _hetero_fig(bad_fracs, Hs, e_m1, e_or, e_cf, gor, gcf):
    ...
    a2.set_xlabel("bad-frame fraction (%)"); a2.set_ylabel("landing-error gain over M1 (cm)")
    a2.legend(); a2.grid(alpha=.3); a2.set_title("M3 advantage  (must clear the floor)")
